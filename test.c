...
int main() {
    ...
    // Dynamic memory allocation using custom malloc_new function
    int *array = (int*)malloc_new(40 * sizeof(int));
    printf("Memory map after malloc_new for array:\n");
    print_memory_map();

    // Allocating memory using custom my_calloc function
    int *array_calloc = (int*)my_calloc(25, sizeof(int));
    printf("Memory map after my_calloc for array_calloc:\n");
    print_memory_map();

    // Reallocating memory using custom my_realloc function
    array = (int*)my_realloc(array, 20 * sizeof(int));
    printf("Memory map after my_realloc for array:\n");
    print_memory_map();

    // Freeing memory using custom free_new function
    free_new(array);
    printf("Memory map after free_new for array:\n");
    print_memory_map();

    // Freeing memory allocated by my_calloc
    free_new(array_calloc);
    printf("Memory map after free_new for array_calloc:\n");
    print_memory_map();
...
// Custom implementation of calloc using malloc_new and memset.
void *my_calloc(size_t num, size_t size) {
    size_t totalSize = num * size;
    void *ptr = malloc_new(totalSize);
    if (ptr) {
...
void *my_realloc(void *ptr, size_t newSize) {
    if (!ptr) {
        // Equivalent to malloc_new if the original pointer is NULL
        return malloc_new(newSize);
    }
    ...
    void *newPtr = malloc_new(newSize);
    if (newPtr) {
